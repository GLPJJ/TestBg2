package simple.view.aligntextview;
...
public class AlignTextView extends AppCompatTextView {
    ...
    // 尾行对齐方式
    public enum Align {
        ALIGN_LEFT, ALIGN_CENTER, ALIGN_RIGHT, ALIGN_SIDE  // 居中，居左，居右,针对段落最后一行
    }
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            if (tailLines.contains(i)) {
                ...
                if (align == Align.ALIGN_CENTER) {
                    ...
                    drawSpacingX += gap;
                } else if (align == Align.ALIGN_SIDE) {
                    if (line.length() > 1) {
                        int count = line.length() - 1;
                        int separate = getWidth() / count;

                        for (int j = 0; j < line.length(); j++) {

                            String subStr = line.substring(j, j + 1);
                            float drawX = separate * j;
                            if (j == 0) {//不处理
                            } else if (j < line.length() - 1)//
                                drawX = drawX - paint.measureText(subStr) / 2;
                            else
                                drawX = drawX - paint.measureText(subStr);

                            canvas.drawText(subStr, drawX, drawY + paddingTop
                                    + textLineSpaceExtra * i, paint);
                        }
                        return;
                    }
                }
