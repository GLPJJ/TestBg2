package simple.view.gridepassword;
 ...
 * @date 15/3/5 21:30
 */
public class GridPasswordView extends LinearLayout implements PasswordView, View.OnFocusChangeListener {
    private final static String TAG = "GridPasswordView";
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (textSize != -1) {
            this.textSize = ToolUtil.Px2Sp(textSize);
        }

        lineWidth = (int) ta.getDimension(R.styleable.gridPasswordView_lineWidth, ToolUtil.Dp2Px(1));
    ...
    public void setPassword(String password) {
        ...
        for (int i = 0; i < pswArr.length; i++) {
            if (i < passwordArr.length) {
                passwordArr[i] = String.valueOf(pswArr[i]);
